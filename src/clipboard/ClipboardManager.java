...
public class ClipboardManager implements IClipboard {
	...
	private void pasteSlots(ArrayList<Slot> slots) {
		...
		for(Slot s:slots) {
			if(!fst) {
				minx=s.getGeometry().getMinX();
				miny=s.getGeometry().getMinY();
			}
			else {
				minx=Math.min(minx,s.getGeometry().getMinX());
				miny=Math.min(miny,s.getGeometry().getMinY());
			}
			fst=true;
		}
		for(Slot s: slots) {
			Slot sn = (Slot)cloneAny(s);
			sn.getGeometry().setXCenter(sn.getGeometry().getXCenter()-minx);
			sn.getGeometry().setYCenter(sn.getGeometry().getYCenter()-miny);
